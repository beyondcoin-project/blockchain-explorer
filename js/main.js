var networksConfigs = {
    'BYND': {
        'name': 'Main Network (BYND)',
        'api': 'https://api.beyondcoin.io',
        'ticker': 'BYND',
        'decimals': 8,
        'hrp': 'bynd'
    }
...
function setTitle(title) {
    document.title = title + ' | Beyondcoin Explorer';
}
...
function displayBlockInfo(data) {
    ...
    content = `
        <div class="info-text">
            <span>Block at depth ${data.height} in the Beyondcoin blockchain</span>
        </div><br />
        <div class="card mb-3">
...
function displayTransactionInfo(data) {
    content = `
        <div class="info-text">
            <span>View information about a Beyondcoin transaction</span>
        </div><br />
        <div class="card mb-3">
            <div class="table-responsive">
                <table class="table table-borderless table-md table-striped mb-0">
                    <tbody>
                        ...
                        ${data.time != undefined ? `
                            <tr>
                                <td><!--Timestamp-->Received Time</td>
                                <td>${timeFormat(data.time)} <!--(<b>${data.time}</b>)--></td>
                            </tr>
                        ` : ''}
                        <tr>
                            <td><!--Height-->Included in Block</td>
                            <td>
                                ${data.blockhash != undefined ? `
                                    <a href="#/block/${data.blockhash}">${data.height}</a><!-- (<b>Confirmations ${data.confirmations}</b>)-->
                                ` : `
                                    <span class="text-danger">Mempool<!--This transaction located in memory pool and not included to blockchain yet!--></span>
                                `}
                            </td>
                        </tr>
                        <tr>
